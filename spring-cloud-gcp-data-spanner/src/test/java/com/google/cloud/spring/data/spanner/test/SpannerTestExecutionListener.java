...
public class SpannerTestExecutionListener implements TestExecutionListener {
  ...
  protected void createDatabaseWithSchema() {
    ...
    if (!this.spannerDatabaseAdminTemplate.databaseExists()) {
      LOGGER.debug(
          this.getClass() + " - Integration database created with schema: " + createStatements);
    ...
    await()
        ...
        .untilAsserted(
            () -> assertThat(this.spannerDatabaseAdminTemplate.databaseExists()).isTrue());
  }

  protected List<String> createSchemaStatements() {
    List<String> list =
        new ArrayList<>(
            this.spannerSchemaUtils.getCreateTableDdlStringsForInterleavedHierarchy(Trade.class));
    list.add(
