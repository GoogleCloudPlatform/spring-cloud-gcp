 ...
 * {@link org.springframework.data.repository.query.parser.PartTree} parsed method definitions.
 *
 * @since 1.1
 */
public final class SpannerStatementQueryExecutor {
  ...
  private static Map<String, Parameter> preparePartTreeSqlTagParameterMap(
    ...
    for (int paramIdx = 0; paramIdx < paramsMetadata.length; paramIdx++) {
      ...
      if (param.getType() != Pageable.class && param.getType() != Sort.class) {
        paramMetadataMap.put(sqlStringAndPlaceholders.getPlaceholders().get(nextPlaceholderIdx++),
            param);
      }
    }
   ...
   *                         eager-Interleaved lists with a single query. Please note, it doesn't
   *                         make sense to pass it as {@code true} when the {@code sql} already
   *                         contains a complete lists of all eager-Interleaved properties generated
   *                         by the method {@link #getColumnsStringForSelect}.
   ...
   *                                  parent entity.
   * @param writeConverter            a converter to convert key values as needed to bind to the
   *                                  query statement.
  ...
  /**
   * Builds a query that returns the rows associated with a key set. If the entity class has
   * {@link com.google.cloud.spring.data.spanner.core.mapping.Where} annotation it will be used to
   * build the query.
   *
   ...
   *                               they cannot be directly mapped to Cloud Spanner supported param
   *                               types. If null then this last-attempt conversion is skipped.
   * @param spannerCustomConverter a converter used to convert params that aren't Cloud Spanner
   *                               native types. if {@code null} then this conversion is not
   *                               attempted.
   * @param params                 the parameters to substitute the tags. The ordering must be the
   *                               same as the tags.
