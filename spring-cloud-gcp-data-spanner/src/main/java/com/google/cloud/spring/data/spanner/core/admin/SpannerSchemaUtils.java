...
public class SpannerSchemaUtils {
  ...
  public SpannerSchemaUtils(
    ...
    Assert.notNull(mappingContext, "A valid mapping context for Cloud Spanner is required.");
  ...
  public String getDropTableDdlString(Class entityClass) {
    return "DROP TABLE " + this.mappingContext.getPersistentEntityOrFail(entityClass).tableName();
  }
  ...
  public Key getKey(Object object) {
    SpannerPersistentEntity persistentEntity =
        this.mappingContext.getPersistentEntityOrFail(object.getClass());
    return (Key)
        persistentEntity.getPropertyAccessor(object).getProperty(persistentEntity.getIdProperty());
  }

  /**
   * Gets the DDL string to create the table for the given entity in Cloud Spanner. This is just one
   * of the possible schemas that can support the given entity type. The specific schema is
   * determined by the configured property type converters used by the read and write methods in
  ...
  public <T> String getCreateTableDdlString(Class<T> entityClass) {
    ...
    StringBuilder stringBuilder =
        new StringBuilder()
            .append("CREATE TABLE ")
            .append(spannerPersistentEntity.tableName())
            .append(" ( ");
  ...
  /**
   * Gets a list of DDL strings to create the tables rooted at the given entity class. The
   * DDL-create strings are ordered in the list starting with the given root class and are
  ...
  public List<String> getCreateTableDdlStringsForInterleavedHierarchy(Class entityClass) {
    ...
    getCreateTableDdlStringsForInterleavedHierarchy(null, entityClass, ddlStrings, new HashSet<>());
    return ddlStrings;
  }

  /**
   * Gets the DDL strings to drop the tables of this entity and all of its sub-entities. The list is
   * given in reverse topological sort, since parent tables cannot be dropped before their children
   * tables.
   *
  ...
  public List<String> getDropTableDdlStringsForInterleavedHierarchy(Class entityClass) {
    List<String> ddlStrings = new ArrayList<>();
    getDropTableDdlStringsForInterleavedHierarchy(entityClass, ddlStrings, new HashSet<>());
    return ddlStrings;
  }
  ...
  String getColumnDdlString(
    ...
    if (ConversionUtils.isIterableNonByteArrayType(columnType)) {
      ...
      spannerJavaType = spannerEntityProcessor.getCorrespondingSpannerJavaType(innerType, true);
    ...
    if (spannerColumnType == Type.Code.JSON) {
      ...
      spannerJavaType = spannerEntityProcessor.getCorrespondingSpannerJavaType(columnType, false);
    }
    ...
    for (SpannerPersistentProperty keyProp : spannerPersistentEntity.getPrimaryKeyProperties()) {
      if (keyProp.isEmbedded()) {
        addPrimaryKeyColumnNames(
            this.mappingContext.getPersistentEntityOrFail(keyProp.getType()), keyStrings);
      } else {
  ...
  private void getCreateTableDdlStringsForInterleavedHierarchy(
      String parentTable, Class entityClass, List<String> ddlStrings, Set<Class> seenClasses) {
    getDdlStringForInterleavedHierarchy(
        parentTable, entityClass, ddlStrings, seenClasses, this::generateSingleStringDdl, false);
  }
  ...
  private void getDropTableDdlStringsForInterleavedHierarchy(
      Class entityClass, List<String> dropStrings, Set<Class> seenClasses) {
    getDdlStringForInterleavedHierarchy(
        null,
        entityClass,
        dropStrings,
        seenClasses,
        (type, unused) -> getDropTableDdlString(type),
        true);
  }

  private void getDdlStringForInterleavedHierarchy(
      String parentTable,
      Class entityClass,
    ...
    ddlStrings.add(
        prependDdlString ? 0 : ddlStrings.size(),
        ...
        (PropertyHandler<SpannerPersistentProperty>)
            spannerPersistentProperty ->
                getDdlStringForInterleavedHierarchy(
                    ...
                    ddlStrings,
                    seenClasses,
                    generateSingleDdlStringFunc,
                    prependDdlString));
  }

  private String getTypeDdlString(
      Type.Code type,
      boolean isArray,
  ...
  private String getTypeDdlStringWithLength(
      Type.Code type, boolean isArray, OptionalLong dataLength) {
