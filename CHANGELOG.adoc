...
** A custom bean for a property takes precedence over auto-configuration. In order to use per-subscription configuration, the custom bean for that property needs to be removed. When using auto-configuration, per-subscription configuration takes precedence over global configuration.
