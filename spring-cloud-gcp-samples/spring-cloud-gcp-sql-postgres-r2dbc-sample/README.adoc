= Spring Framework on Google Cloud PostgreSQL R2DBC Sample

This code sample demonstrates how to connect to a PostgreSQL Google Cloud SQL instance using the link:../../spring-cloud-gcp-starters/spring-cloud-gcp-starter-sql-postgres-r2dbc[Spring Framework on Google Cloud PostgreSQL R2DBC Starter].
...
NOTE: The sample initializes the database using `CREATE TABLE IF NOT EXISTS users` and populates it with sample rows without dropping the table on shutdown. The `users` table will persist in your Cloud SQL instance after running the sample. If you wish to remove the table, you can drop it manually using your SQL client (`DROP TABLE users;`).
