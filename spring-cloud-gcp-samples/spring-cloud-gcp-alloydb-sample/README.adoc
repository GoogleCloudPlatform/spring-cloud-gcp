...
NOTE: The sample initializes the database using `CREATE TABLE IF NOT EXISTS users` and populates it with sample rows without dropping the table on shutdown. The `users` table will persist in your AlloyDB database after running the sample. If you wish to remove the table, you can drop it manually using your SQL client (`DROP TABLE users;`).
