 ...
 * TestContainers, and then our app's docker image will connect to that firestore to persist/fetch data. Test scenario
 * is almost exactly the same as {@link FirestoreSampleApplicationIntegrationTests}.
 */
@Testcontainers
@Tag("native")
public class FirestoreSampleApplicationNativeIntegrationTests {
	...
	private void verifyUserPersisted(User user) throws InterruptedException, ExecutionException {
		...
		assertThat(storedDoc)
				.isNotNull()
				.containsEntry("age", (long) user.getAge());
	...
	private static class MyAppNativeDockerContainer extends GenericContainer<MyAppNativeDockerContainer> {
		...
		MyAppNativeDockerContainer(FirestoreEmulatorContainer emulator) {
			...
			withEnv("SPRING_CLOUD_GCP_FIRESTORE_HOST_PORT", "host.testcontainers.internal:" + emulator.getMappedPort(8080));
			withEnv("SPRING_CLOUD_GCP_FIRESTORE_PROJECT_ID", MY_GCP_PROJECT_ID);
			withExposedPorts(8080);
			waitingFor(Wait.forHttp("/"));
			withLogConsumer(outputFrame -> System.out.println("MYAPP-DOCKER-OUTPUT	" + outputFrame.getUtf8String().trim()));
		}
