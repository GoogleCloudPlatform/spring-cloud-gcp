...
public class DatastoreRepositoryExample {
  ...
  private void retrieveAndPrintSingers() {
    System.out.println(
        "The kind for singers has been cleared and "
            + this.singerRepository.count()
            + " new singers have been inserted:");
    ...
    Iterable<Singer> singers =
        this.singerRepository.findAll(
            Example.of(
                new Singer(null, null, "Doe", null),
  ...
  private void createRelationshipsInTransaction(Singer singerA, Singer singerB) {
    ...
    this.transactionalRepositoryService.createAndSaveSingerRelationshipsInTransaction(
        ...
        new HashSet<>(Arrays.asList(new Instrument("recorder"), new Instrument("cow bell"))));
    ...
    this.singerRepository.performTransaction(
        transactionRepository -> {
          singerB.setFirstBand(band3);
          singerB.setBands(Arrays.asList(band3, band2));
          singerB.setPersonalInstruments(
              new HashSet<>(Arrays.asList(new Instrument("triangle"), new Instrument("marimba"))));
