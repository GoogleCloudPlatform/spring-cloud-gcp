...
class DatastoreSampleApplicationIntegrationTests {
  ...
  void basicTest() throws Exception {
    ...
    assertThat(
            this.singerRepository.findById("singer2").get().getPersonalInstruments().stream()
    ...
    assertThat(
            singer3.getPersonalInstruments().stream()
                .map(Instrument::getType)
                .collect(Collectors.toList()))
  ...
  void testCompoundKeyRestResource() throws IOException {
    ...
    Map<String, Object> parsedResponse =
        this.mapper.readValue(
            allInstrumentsResponse, new TypeReference<HashMap<String, Object>>() {});
    Map instrument =
        (Map) ((List) ((Map) parsedResponse.get("_embedded")).get("instruments")).get(0);
    String instrumentUrl =
        (String) ((Map) ((Map) instrument.get("_links")).get("instrument")).get("href");
  ...
  private List<Singer> getSingers(String url) throws java.io.IOException {
    ...
    List<Map<String, Object>> singerMaps =
        (List<Map<String, Object>>)
            ((Map<String, Object>) parsedResponse.get("_embedded")).get("singers");
