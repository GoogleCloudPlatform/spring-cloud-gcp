...
a. Put `kms://spring-cloud-gcp/key-id` as the `Key Id`, add some text and press to `Encrypt Text`.
...
b. Put `kms://spring-cloud-gcp/key-id` as the `Key Id` and press `Decrypt Text`.
