...
. Increment the project base _patch_ version. For example, from `2.0.4-SNAPSHOT` to `2.0.5-SNAPSHOT`, do:
```
$ ./mvnw versions:set -DprocessAllModules -DnewVersion=2.0.5-SNAPSHOT
```
...
----
+
Example: If you published the javadocs for version `2.0.5-SNAPSHOT`, then the URL would be: https://googleapis.dev/java/spring-cloud-gcp/2.0.5-SNAPSHOT/index.html
