...
/**
 * Auto-configuration for {@link FleetRoutingClient}.
 *
...
@AutoConfiguration
@ConditionalOnClass(FleetRoutingClient.class)
@ConditionalOnProperty(
    value = "com.google.cloud.optimization.v1.spring.auto.fleet-routing.enabled",
    matchIfMissing = true)
@EnableConfigurationProperties(FleetRoutingSpringProperties.class)
public class FleetRoutingSpringAutoConfiguration {
  private final FleetRoutingSpringProperties clientProperties;
  private final CredentialsProvider credentialsProvider;
  private static final Log LOGGER = LogFactory.getLog(FleetRoutingSpringAutoConfiguration.class);

  protected FleetRoutingSpringAutoConfiguration(
      FleetRoutingSpringProperties clientProperties, CredentialsProvider credentialsProvider)
      throws IOException {
  ...
  public TransportChannelProvider defaultFleetRoutingTransportChannelProvider() {
    return FleetRoutingSettings.defaultTransportChannelProvider();
  }

  /**
   * Provides a FleetRoutingClient bean configured to use the default credentials provider (obtained
   * with fleetRoutingCredentials()) and its default transport channel provider
   * (defaultFleetRoutingTransportChannelProvider()). It also configures the quota project ID if
  ...
  public FleetRoutingSettings fleetRoutingSettings(
    ...
    if (serviceRetry != null) {
      ...
      RetrySettings batchOptimizeToursRetrySettings =
          RetryUtil.updateRetrySettings(
              clientSettingsBuilder.batchOptimizeToursSettings().getRetrySettings(), serviceRetry);
      clientSettingsBuilder
          .batchOptimizeToursSettings()
          .setRetrySettings(batchOptimizeToursRetrySettings);
    ...
    Retry batchOptimizeToursRetry = clientProperties.getBatchOptimizeToursRetry();
    if (batchOptimizeToursRetry != null) {
      RetrySettings batchOptimizeToursRetrySettings =
          RetryUtil.updateRetrySettings(
              clientSettingsBuilder.batchOptimizeToursSettings().getRetrySettings(),
              batchOptimizeToursRetry);
      clientSettingsBuilder
          .batchOptimizeToursSettings()
          .setRetrySettings(batchOptimizeToursRetrySettings);
      if (LOGGER.isTraceEnabled()) {
        LOGGER.trace(
            "Configured method-level retry settings for batchOptimizeTours from properties.");
      }
    }
