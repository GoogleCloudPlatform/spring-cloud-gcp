...
@ConditionalOnProperty(
    value = "com.google.cloud.aiplatform.v1.spring.auto.model-service.enabled",
    matchIfMissing = true)
...
public class ModelServiceSpringAutoConfiguration {
  ...
  /**
   * Provides a ModelServiceClient bean configured to use the default credentials provider (obtained
   * with modelServiceCredentials()) and its default transport channel provider
   * (defaultModelServiceTransportChannelProvider()). It also configures the quota project ID if
  ...
  public ModelServiceSettings modelServiceSettings(
    ...
    if (serviceRetry != null) {
      RetrySettings uploadModelRetrySettings =
          RetryUtil.updateRetrySettings(
              clientSettingsBuilder.uploadModelSettings().getRetrySettings(), serviceRetry);
      clientSettingsBuilder.uploadModelSettings().setRetrySettings(uploadModelRetrySettings);
      ...
      RetrySettings deleteModelVersionRetrySettings =
          RetryUtil.updateRetrySettings(
              clientSettingsBuilder.deleteModelVersionSettings().getRetrySettings(), serviceRetry);
      clientSettingsBuilder
          .deleteModelVersionSettings()
          .setRetrySettings(deleteModelVersionRetrySettings);
    ...
    Retry uploadModelRetry = clientProperties.getUploadModelRetry();
    if (uploadModelRetry != null) {
      RetrySettings uploadModelRetrySettings =
          RetryUtil.updateRetrySettings(
              clientSettingsBuilder.uploadModelSettings().getRetrySettings(), uploadModelRetry);
      clientSettingsBuilder.uploadModelSettings().setRetrySettings(uploadModelRetrySettings);
      if (LOGGER.isTraceEnabled()) {
        LOGGER.trace("Configured method-level retry settings for uploadModel from properties.");
      }
    }
    ...
    Retry deleteModelVersionRetry = clientProperties.getDeleteModelVersionRetry();
    if (deleteModelVersionRetry != null) {
      RetrySettings deleteModelVersionRetrySettings =
          RetryUtil.updateRetrySettings(
              clientSettingsBuilder.deleteModelVersionSettings().getRetrySettings(),
              deleteModelVersionRetry);
      clientSettingsBuilder
          .deleteModelVersionSettings()
          .setRetrySettings(deleteModelVersionRetrySettings);
      if (LOGGER.isTraceEnabled()) {
        LOGGER.trace(
            "Configured method-level retry settings for deleteModelVersion from properties.");
      }
    }
