...
@ConditionalOnProperty(
    value = "com.google.cloud.trace.v2.trace-service.enabled",
    matchIfMissing = true)
@EnableConfigurationProperties(TraceServiceSpringProperties.class)
public class TraceServiceSpringAutoConfiguration {
  ...
  protected TraceServiceSpringAutoConfiguration(
      TraceServiceSpringProperties clientProperties, CredentialsProvider credentialsProvider)
      throws IOException {
   ...
   */
  @Bean
  @ConditionalOnMissingBean(name = "defaultTraceServiceTransportChannelProvider")
  public TransportChannelProvider defaultTraceServiceTransportChannelProvider() {
    if (this.clientProperties.getUseRest()) {
      return TraceServiceSettings.defaultHttpJsonTransportProviderBuilder().build();
    }
    return TraceServiceSettings.defaultTransportChannelProvider();
  }

  /**
   * Provides a TraceServiceSettings bean configured to use a DefaultCredentialsProvider and the
   * client library's default transport channel provider
   * (defaultTraceServiceTransportChannelProvider()). It also configures the quota project ID and
