...
/**
 * Auto-configuration for {@link ApiKeysClient}.
 *
...
@ConditionalOnClass(ApiKeysClient.class)
@ConditionalOnProperty(value = "com.google.api.apikeys.v2.api-keys.enabled", matchIfMissing = true)
@EnableConfigurationProperties(ApiKeysSpringProperties.class)
public class ApiKeysSpringAutoConfiguration {
  private final ApiKeysSpringProperties clientProperties;
  private final CredentialsProvider credentialsProvider;
  private static final Log LOGGER = LogFactory.getLog(ApiKeysSpringAutoConfiguration.class);

  protected ApiKeysSpringAutoConfiguration(
      ApiKeysSpringProperties clientProperties, CredentialsProvider credentialsProvider)
      throws IOException {
    this.clientProperties = clientProperties;
    if (this.clientProperties.getCredentials().hasKey()) {
      if (LOGGER.isTraceEnabled()) {
        LOGGER.trace("Using credentials from ApiKeys-specific configuration");
      }
   ...
   */
  @Bean
  @ConditionalOnMissingBean(name = "defaultApiKeysTransportChannelProvider")
  public TransportChannelProvider defaultApiKeysTransportChannelProvider() {
    if (this.clientProperties.getUseRest()) {
      return ApiKeysSettings.defaultHttpJsonTransportProviderBuilder().build();
    }
    return ApiKeysSettings.defaultTransportChannelProvider();
  }

  /**
   * Provides a ApiKeysSettings bean configured to use a DefaultCredentialsProvider and the client
   * library's default transport channel provider (defaultApiKeysTransportChannelProvider()). It
