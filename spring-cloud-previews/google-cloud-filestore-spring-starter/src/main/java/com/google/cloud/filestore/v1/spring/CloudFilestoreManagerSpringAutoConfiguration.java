...
@ConditionalOnProperty(
    value = "com.google.cloud.filestore.v1.spring.auto.cloud-filestore-manager.enabled",
    matchIfMissing = true)
...
public class CloudFilestoreManagerSpringAutoConfiguration {
  ...
  /**
   * Provides a CloudFilestoreManagerClient bean configured to use the default credentials provider
   * (obtained with cloudFilestoreManagerCredentials()) and its default transport channel provider
  ...
  public CloudFilestoreManagerSettings cloudFilestoreManagerSettings(
    ...
    if (serviceRetry != null) {
      ...
      RetrySettings restoreInstanceRetrySettings =
          RetryUtil.updateRetrySettings(
              clientSettingsBuilder.restoreInstanceSettings().getRetrySettings(), serviceRetry);
      clientSettingsBuilder
          .restoreInstanceSettings()
          .setRetrySettings(restoreInstanceRetrySettings);
      ...
      RetrySettings updateBackupRetrySettings =
          RetryUtil.updateRetrySettings(
              clientSettingsBuilder.updateBackupSettings().getRetrySettings(), serviceRetry);
      clientSettingsBuilder.updateBackupSettings().setRetrySettings(updateBackupRetrySettings);
    ...
    Retry restoreInstanceRetry = clientProperties.getRestoreInstanceRetry();
    if (restoreInstanceRetry != null) {
      RetrySettings restoreInstanceRetrySettings =
          RetryUtil.updateRetrySettings(
              clientSettingsBuilder.restoreInstanceSettings().getRetrySettings(),
              restoreInstanceRetry);
      clientSettingsBuilder
          .restoreInstanceSettings()
          .setRetrySettings(restoreInstanceRetrySettings);
      if (LOGGER.isTraceEnabled()) {
        LOGGER.trace("Configured method-level retry settings for restoreInstance from properties.");
      }
    }
    ...
    Retry updateBackupRetry = clientProperties.getUpdateBackupRetry();
    if (updateBackupRetry != null) {
      RetrySettings updateBackupRetrySettings =
          RetryUtil.updateRetrySettings(
              clientSettingsBuilder.updateBackupSettings().getRetrySettings(), updateBackupRetry);
      clientSettingsBuilder.updateBackupSettings().setRetrySettings(updateBackupRetrySettings);
      if (LOGGER.isTraceEnabled()) {
        LOGGER.trace("Configured method-level retry settings for updateBackup from properties.");
      }
    }
