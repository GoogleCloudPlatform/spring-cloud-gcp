...
Cloud Memorystore is compatible with the Redis protocol, allowing easy integration with https://docs.spring.io/spring-boot/docs/3.1.x/reference/html/io.html#io.caching[Spring Caching].

All you have to do is create a Cloud Memorystore instance and use its IP address in `application.properties` file as `spring.data.redis.host` property value. (Read more about this property in https://docs.spring.io/spring-boot/docs/3.1.x/reference/htmlsingle/#appendix.application-properties.data[Spring Boot documentation]. )
