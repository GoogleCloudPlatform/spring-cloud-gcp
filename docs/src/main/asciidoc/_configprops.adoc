...
|spring.cloud.gcp.pubsub.publisher.batching.delay-threshold-duration |  | The delay threshold to use for batching. After this amount of time has elapsed (counting from the first element added), the elements will be wrapped up in a batch and sent.
