...
Spring Cloud GCP offers a utility template class `KmsTemplate` which allows you to conveniently specify secrets for encryption/decryption.
...
The `com.google.cloud.spring.kms.KmsTemplate` class provided by the autoconfiguration is the entrypoint to using Spring Cloud GCP support for Google KMS. This class allows you to specify a Cloud KMS key in your project via a URI string (format described below) and perform encryption/decryption with it.
...
[source]
----
 1. Shortest form - specify the key by location ID, key ring ID, and key ID.
