...
The health indicator validates a subscriber health by checking the subscription's message backlog and the last processed message.
A subscription's backlog is retrieving using Google Cloud's Monitoring Metrics. The metric used is the `num_undelivered_messages` for a subscription.
