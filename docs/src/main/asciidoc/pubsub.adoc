...
Subscriber settings can also be set on a per-subscription basis.
A custom configuration (set through a setter in `DefaultSubscriberFactor` or a custom bean) will take precedence over auto-configuration.
When using auto-configuration, the custom bean for a property needs to be removed in order for the global/per-subscription configuration to be taken into account.
If both global and per-subscription configurations are set then the per-subscription configuration will be used.
