...
Subscriber settings can also be set on a per-subscription basis. A custom configuration (set through a setter in `DefaultSubscriberFactor` or a custom bean) will take precedence over auto-configuration. When using auto-configuration, if one wishes to use per-subscription configuration for a property then they need to avoid using a custom bean for that property. If both global and per-subscription configurations are provided then the per-subscription configuration will be used. However, if subscription-specific property is not specified then the global or default configuration will be used.
...
Maximum number of outstanding elements to keep in memory before enforcing flow control. If the per-subscription configuration for this field is not provided then the global configuration is picked, otherwise, the default is picked. | No | unlimited
| `spring.cloud.gcp.pubsub.subscription.[subscription-name].flow-control.max-outstanding-request-bytes`|
Maximum number of outstanding bytes to keep in memory before enforcing flow control. If the per-subscription configuration for this field is not provided then the global configuration is picked, otherwise, the default is picked. | No | unlimited
| `spring.cloud.gcp.pubsub.subscription.[subscription-name].flow-control.limit-exceeded-behavior`|
The behavior when the specified limits are exceeded. If the per-subscription configuration for this field is not provided then the global configuration is picked, otherwise, the default is picked. | No | Block
|===
...
TotalTimeout has ultimate control over how long the logic should keep trying the remote call until it gives up completely. The higher the total timeout, the more retries can be attempted. If the per-subscription configuration for this field is not provided then the global configuration is picked, otherwise, the default is picked. | No | 0
| `spring.cloud.gcp.pubsub.subscription.[subscription-name].retry.initial-retry-delay-second`|
InitialRetryDelay controls the delay before the first retry.
Subsequent retries will use this value adjusted according to the RetryDelayMultiplier. If the per-subscription configuration for this field is not provided then the global configuration is picked, otherwise, the default is picked. | No | 0
| `spring.cloud.gcp.pubsub.subscription.[subscription-name].retry.retry-delay-multiplier`|
RetryDelayMultiplier controls the change in retry delay.
The retry delay of the previous call is multiplied by the RetryDelayMultiplier to calculate the retry delay for the next call. If the per-subscription configuration for this field is not provided then the global configuration is picked, otherwise, the default is picked. | No | 1
| `spring.cloud.gcp.pubsub.subscription.[subscription-name].retry.max-retry-delay-seconds`|
MaxRetryDelay puts a limit on the value of the retry delay, so that the RetryDelayMultiplier
can't increase the retry delay higher than this amount. If the per-subscription configuration for this field is not provided then the global configuration is picked, otherwise, the default is picked. | No | 0
| `spring.cloud.gcp.pubsub.subscription.[subscription-name].retry.max-attempts`|
MaxAttempts defines the maximum number of attempts to perform.
If this value is greater than 0, and the number of attempts reaches this limit, the logic will give up retrying even if the total retry time is still lower than TotalTimeout. If the per-subscription configuration for this field is not provided then the global configuration is picked, otherwise, the default is picked. | No | 0
| `spring.cloud.gcp.pubsub.subscription.[subscription-name].retry.jittered`|Jitter determines if the delay time should be randomized. If the per-subscription configuration for this field is not provided then the global configuration is picked, otherwise, the default is picked. | No | true
| `spring.cloud.gcp.pubsub.subscription.[subscription-name].retry.initial-rpc-timeout-seconds`|
InitialRpcTimeout controls the timeout for the initial RPC.
Subsequent calls will use this value adjusted according to the RpcTimeoutMultiplier. If the per-subscription configuration for this field is not provided then the global configuration is picked, otherwise, the default is picked. | No |0
| `spring.cloud.gcp.pubsub.subscription.[subscription-name].retry.rpc-timeout-multiplier`|
RpcTimeoutMultiplier controls the change in RPC timeout.
The timeout of the previous call is multiplied by the RpcTimeoutMultiplier to calculate the timeout for the next call. If the per-subscription configuration for this field is not provided then the global configuration is picked, otherwise, the default is picked. | No | 1
| `spring.cloud.gcp.pubsub.subscription.[subscription-name].retry.max-rpc-timeout-seconds`|
MaxRpcTimeout puts a limit on the value of the RPC timeout, so that the RpcTimeoutMultiplier
can't increase the RPC timeout higher than this amount. If the per-subscription configuration for this field is not provided then the global configuration is picked, otherwise, the default is picked. | No | 0
|===
